...
```text
请审计这套 Markstream 接入。
重点检查：包和 peers 是否匹配、CSS 顺序是否正确、Tailwind/UnoCSS layer 是否安全、KaTeX CSS 是否缺失、stream-diffs / Mermaid / D2 是否在正确的客户端边界内初始化。
...
Check whether the package and peers match the actual features, whether CSS order and Tailwind or UnoCSS layers are safe, whether KaTeX CSS is missing, and whether stream-diffs, Mermaid, or D2 are initialized behind the correct client-only boundaries.
