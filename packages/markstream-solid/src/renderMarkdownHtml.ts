...
const DEFAULT_CACHE_KEY = 'markstream-solid-html'
...
export function renderMarkdownToHtml(input: MarkstreamSolidRenderOptions): string {
...
function resolveParsedNodes(input: MarkstreamSolidRenderOptions, ctx: RenderContext) {
...
function renderTextNode(node: RenderableMarkdownNode): string {
  ...
  const className = centered
    ? 'markstream-solid-text-node markstream-solid-text--centered'
    : 'markstream-solid-text-node'
