...
export function setMermaidWorker(nextWorker: Worker) {
  ...
  worker.onerror = (event: ErrorEvent) => {
    if (worker !== current)
      return
    if (rpcMap.size === 0) {
      console.debug?.('[markstream-solid:mermaidWorkerClient] Worker error (idle):', event?.message || event)
      return
    }
    try {
      if (debugClient)
        console.error('[markstream-solid:mermaidWorkerClient] Worker error:', event?.message || event)
      else
        console.debug?.('[markstream-solid:mermaidWorkerClient] Worker error:', event?.message || event)
    }
    catch {
  ...
  ;(worker as any).onmessageerror = (event: MessageEvent) => {
    if (worker !== current)
      return
    if (rpcMap.size === 0) {
      console.debug?.('[markstream-solid:mermaidWorkerClient] Worker messageerror (idle):', event)
      return
    }
    for (const [, active] of rpcMap.entries())
...
export function hasMermaidWorker() {
  return worker !== null
}
...
function ensureWorker() {
  ...
  workerInitError = new Error('[markstream-solid:mermaidWorkerClient] No worker instance set. Please inject a Worker via setMermaidWorker().')
