...
| `NodeRenderer.svelte` | `src/components/NodeRenderer.tsx` | content/nodes、final、smooth stream、context、批量节点调度 | 部分实现 | 解析、Solid DOM、append DOM 身份、完成态 idle/RAF 批量调度、SSR 输出和同树 hydration 回归通过；浏览器用例待补 |
...
| `CodeBlockNode.svelte` | `src/components/CodeBlockNode.tsx` | 单/双栏实例、增量 update、theme、可选 runtime fallback、卸载清理、复制/预览/展开/折叠/字体大小控件、non-diff fallback 行号，以及流式不完整语言标识延迟 | 部分实现 | 类型检查、控件挂载、fallback 行号、可选 runtime 降级、语言前缀收敛；Chromium 中真实 runtime 保持 shell、受限滚动位置和原生选区，并验证字体从 12px 调整至 13px |
...
| `MathInlineNode.svelte`、`MathBlockNode.svelte` | `src/components/MathNodes.tsx` | KaTeX 可选加载、async generation guard、文本 fallback；注入 worker 时的背压渲染、取消与主线程降级 | 部分实现 | 类型检查、异步过期、worker 优先渲染、清理待处理请求、inline/block enhancement 取消及可选依赖缺失回归，以及 Chromium 中真实 KaTeX DOM 和 `katexRenderer.worker` 请求通过；缓存策略待补 |
| `MermaidBlockNode.svelte` | `src/components/MermaidBlockNode.tsx` | 可选 loader、主题、SVG 净化、async generation guard、源文本 fallback、模式、复制、导出、全屏、折叠和缩放控件，以及 gated `bindFunctions` 交互；流式状态可使用注入的 worker 选择可渲染前缀 | 部分实现 | 类型检查、异步过期、Svelte 兼容预览高度默认值和 300ms 流式节流、工具栏 source/collapse/zoom、bindFunctions opt-in 及 worker 前缀回归，以及 Chromium 中真实 Mermaid SVG 与 parser worker 请求通过 |
| `D2BlockNode.svelte` | `src/components/D2BlockNode.tsx` | 可选 loader、compile/render、主题和深色 override、SVG 净化、async generation guard、源文本 fallback、模式切换、复制、导出和折叠控件，以及流式 progressive render 节流 | 部分实现 | 类型检查、过期异步 D2 结果、渐进流式合并、深色 options、工具栏 source/collapse 及 SVG 尚未产出时源码 fallback 回归、Chromium 中真实 D2 SVG 输出通过 |
...
`HtmlPreviewFrame.svelte`、`Tooltip.svelte`：已移植；Tooltip 定位与清理已回归验证，Chromium 已验证 HTML Preview iframe 与 source 内容。`MathBlockNode.svelte`、`MathInlineNode.svelte`：已实现 KaTeX 渲染、async fallback，以及注入 worker 时的背压渲染与取消；Chromium 已验证实际 worker 注入，可选依赖缺失、inline/block enhancement 取消与 worker 清理均有回归，缓存策略待验证。
...
`InfographicBlockNode.svelte`：已实现 loader、流式 source 更新时的实例复用、销毁、异步过期 guard、安全 SVG、错误回退、渐进式预览，以及 source/复制/导出/全屏/折叠/缩放工具栏。`MermaidBlockNode.svelte`：部分实现，已具备 SVG 预览、安全净化、主题、错误回退、异步过期 guard、受控 `bindFunctions` 交互和 source/复制/导出/全屏/折叠/缩放工具栏；流式状态支持注入 worker 的完整语法验证与可渲染前缀回退，Chromium 已验证实际 parser worker 注入。`D2BlockNode.svelte`：部分实现，已具备 compile/render、安全净化、主题和深色覆盖、异步过期 guard、source/复制/导出/折叠工具栏，以及流式渐进渲染节流；交互仍待补。`CodeBlockNode.svelte`：部分实现，已具备实例复用、single/diff 切换、异步过期 guard、卸载清理，以及复制/预览/展开/折叠/字体大小工具栏；流式语言前缀延迟、真实 runtime 的布局、选区和滚动保持均已验证。`NodeRenderer.svelte`：部分实现，已具备批量调度、HTML 增强、代码实例 key 和资源释放；浏览器用例仍可扩展。`NodeOutlet.svelte` 与 `RenderChildren.svelte`：已实现；后者的 append、插入、删除和重排均有位置槽位 DOM 身份回归。
...
- `CodeBlockNode` 已接入可选 runtime 的实例复用与单/差异视图切换；工具栏、预览、语言前缀收敛以及真实 runtime 的选区/滚动保持均已回归。
- HTML sanitize 与 enhancement 的 owner/dispose 模型已移植；实际 browser 中的增强取消与第三方 runtime 交互仍待验证。
- 后续可补充完整 HTML 字符串输出对照、D2 交互和更广泛的 NodeRenderer 浏览器用例。
