import type { NodeRendererCodeBlockProps, NodeRendererD2Props, NodeRendererInfographicProps, NodeRendererMermaidProps } from './node-helpers'
...
async function renderKatex(root: HTMLElement, isActive: () => boolean) {
  ...
  for (const node of inlineNodes) {
    ...
    try {
      const markup = await renderKatexMarkup(source, node.dataset.display === 'block')
      if (!isActive())
        return
      writeKatexMarkup(node, renderTarget, markup)
      node.dataset.markstreamKatexSource = source
    }
    catch {
      clearKatexMarkup(node, renderTarget)
  ...
  for (const node of blockNodes) {
    ...
    try {
      const markup = await renderKatexMarkup(source, true)
      if (!isActive())
        return
      writeKatexMarkup(node, renderTarget, markup, {
...
async function renderMermaid(
  ...
  for (const codeNode of codeNodes) {
    ...
    try {
      ...
      const renderId = `markstream-solid-mermaid-${++mermaidRenderId}`
      ...
      shell.body.classList.add('markstream-solid-mermaid')
...
async function canParseMermaidWithFallback(
  ...
  const renderId = `markstream-solid-mermaid-parse-${++mermaidRenderId}`
...
async function renderCodeBlock(
  ...
  for (const pre of preNodes) {
    ...
    shell.body.classList.add('markstream-solid-enhanced-block__body--code')
...
function createEnhancedBlockShell(
  ...
  wrapper.className = `markstream-solid-enhanced-block markstream-solid-enhanced-block--${kind}`

  if (shellOptions.showHeader !== false) {
    const header = document.createElement('div')
    header.className = 'markstream-solid-enhanced-block__header'

    const badge = document.createElement('span')
    badge.className = 'markstream-solid-enhanced-block__badge'
  ...
  body.className = 'markstream-solid-enhanced-block__body'
  wrapper.appendChild(body)

  if (showSourceDetails) {
    const details = document.createElement('details')
    details.className = 'markstream-solid-enhanced-block__details'
    ...
    sourcePre.className = 'markstream-solid-enhanced-block__source'
...
function createHeaderActions(source: string, options: EnhanceRenderedHtmlOptions) {
  ...
  actions.className = 'markstream-solid-enhanced-block__actions'

  const copyButton = document.createElement('button')
  copyButton.type = 'button'
  copyButton.className = 'markstream-solid-enhanced-block__action'
