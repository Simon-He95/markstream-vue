...
本地文件系统路径不会被特殊处理。`/Users/alice/image.png` 会被当成站点绝对 URL，`file://` 默认会被阻断。桌面应用应通过自定义协议或本地服务暴露受信任文件，再用自定义 `ImageNode` 改写 Markdown 图片；参见 [ImageNode — 桌面应用中的本地文件图片](/zh/guide/image-node#桌面应用中的本地文件图片)。
