...
export function clearKaTeXWorker() {
  ...
  for (const [, active] of pending) {
    clearTimeout(active.timeoutId)
    const error = new Error('KaTeX worker cleared')
...
export function hasKaTeXWorker() {
  return worker !== null
}
...
function ensureWorker() {
  ...
  workerInitError = new Error('[markstream-solid:katexWorkerClient] No worker instance set. Please inject a Worker via setKaTeXWorker().')
