...
| Packed consumer at `/tmp/markstream-solid-current-consumer` (**historical path**; this playground-migration run does not reuse it) | passed; installed the freshly packed `markstream-solid-2.0.11.tgz` and built with Vite using the public default export and `index.css`; optional peer fallbacks were bundled as expected |
| `pnpm lint` | passed |
...
The local playground streaming flow recorded here was **historical** (no `playground-solid/` source in git `1f51247e`). Do not treat it as this playground-migration run. See `playground-solid/VERIFICATION.md`.
